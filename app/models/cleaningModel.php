...
class cleaningModel extends Model{
     ...
     */
    public function create($flatId, $title, $freq, $wday, $start) {
    ...
    public function getTaskList($flatId) {
        ...
        $res = $this->db->select('a.id, a.flats_id, a.title, a.start, b.description, c.day',
                'cleanings a, frequencies b, wdays c', 
                'a.frequencies_id = b.id AND a.wdays_id = c.id AND flats_id = :flatId', $bind);
        if (!empty($res)) {
            return $res;
        } else {
